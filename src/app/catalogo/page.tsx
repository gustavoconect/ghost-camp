                ...
                <div className="w-full max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 pb-20 sm:pb-32">
                    ...
                    <div className="mb-8 md:mb-12">
                        ...
                        <div className="flex w-full justify-center mb-6">
                            <div className="inline-flex items-center gap-2 px-4 py-2 border border-blue-500/20 bg-blue-500/5 rounded-full">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-8">
                            ...
                            <div className="relative w-full md:w-72 lg:w-80 shrink-0">
                                <label htmlFor="catalog-search" className="sr-only">Buscar equipamento</label>
                                ...
                                <input
                                    ...
                                    className="w-full min-h-[44px] h-11 bg-slate-900/60 backdrop-blur-sm border border-slate-700/50 rounded-xl pl-10 pr-4 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-colors"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-8 pb-6 border-b border-slate-800/60">
                        ...
                        {categories.length > 0 ? (
                            <div className="flex gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto pb-1 sm:pb-0">
                                <button
                                    ...
                                    className={`text-xs font-bold transition-all shrink-0 border whitespace-nowrap min-h-[44px] h-11 px-5 rounded-xl flex items-center justify-center cursor-pointer ${selectedCategory === 'all'
                                        ? 'bg-blue-600 border-blue-500 text-white shadow-[0_0_15px_rgba(37,99,235,0.3)]'
                                        : 'bg-transparent border-slate-700/60 text-slate-300 hover:text-white hover:border-slate-500'
                                        }`}
                                >
                                    Todos
                                </button>
                                {categories.map((cat) => (
                                    <button
                                        ...
                                        className={`text-xs font-bold transition-all shrink-0 border whitespace-nowrap min-h-[44px] h-11 px-5 rounded-xl flex items-center justify-center cursor-pointer ${selectedCategory === cat.id
                                            ? 'bg-blue-600 border-blue-500 text-white shadow-[0_0_15px_rgba(37,99,235,0.3)]'
                                            : 'bg-transparent border-slate-700/60 text-slate-300 hover:text-white hover:border-slate-500'
                                            }`}
                                    >
                        ...
                        <div className="relative shrink-0 w-full sm:w-auto">
                            <label htmlFor="sort-select" className="sr-only">Ordenar equipamentos por</label>
                            <select
                                id="sort-select"
                                value={sortBy}
                                aria-label="Ordenar equipamentos por"
                                onChange={(e) => setSortBy(e.target.value as any)}
                                className="w-full sm:w-auto appearance-none bg-slate-900/60 border border-slate-700/50 rounded-xl text-xs text-slate-200 font-bold focus:outline-none focus:border-blue-500 cursor-pointer min-h-[44px] h-11 pl-4 pr-10 transition-colors hover:border-slate-500"
                            >
                                <option value="default" className="bg-slate-900 text-white">Relevância</option>
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32">
                            ...
                            <p className="text-slate-400 font-bold uppercase tracking-widest text-xs animate-pulse">Carregando catálogo...</p>
                        </div>
                    ) : filteredAndSorted.length === 0 ? (
                        <div className="text-center py-24 sm:py-32 glass-card max-w-2xl mx-auto rounded-3xl">
                            <div className="w-16 h-16 bg-slate-900 mx-auto mb-6 flex items-center justify-center border border-slate-800 rounded-2xl">
                            ...
                            <p className="text-slate-300 px-6 font-medium text-sm max-w-md mx-auto leading-relaxed">
